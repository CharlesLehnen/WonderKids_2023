...
# Null hypothesis: all topics are equally preferred
expected_votes <- rep(sum(data$Votes) / nrow(data), nrow(data))
chisq_test <- chisq.test(data$Votes, p = rep(1 / nrow(data), nrow(data)))

# Print Chi-squared test results
print("Chi-Squared Test for Favorite Topics:")
print(chisq_test)
...
ggplot(data, aes(ymax = ymax, ymin = ymin, xmax = 4, xmin = 3, fill = Topic)) +
  ...
  geom_text(aes(x = 3.5, y = LabelPosition, label = str_wrap(Label, width = 10)), 
            size = 3) + 
  scale_fill_manual(values = wes_palette("Darjeeling1", n = nrow(data), type = "continuous")) +
  coord_polar(theta = "y") +
  xlim(c(2, 4)) +
  theme_void() +
  theme(
